export const toHexBinary = (s) => {
    const l = s.length, r = new Array(l);
    for (let i = 0; i < l; i++) {
        r[i] = ('0' + s.charCodeAt(i).toString(16)).slice(-2);
    }
    return r.join('');
};
export const toBase64Binary = (s) => {
    const l = s.length, r = new Array((l + 2) / 3 << 2), t = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
    let p = (3 - l % 3) % 3, j = 0, i = 0, n;
    s += '\x00\x00';
    while (i < l) {
...
export const toMonoImage = (imgdata, s, g) => {
    const x = String.fromCharCode, m8 = [
    ...
    ], d = imgdata.data, w = imgdata.width, h = imgdata.height, r = new Array((w + 7 >> 3) * h), e = [];
    let n = 0, p = 0, q = 0, t = 128, e1, e2, b, v, f, i, j;
    if (s == 1) {
        i = w;
        while (i--) {
            e.push(0);
...
export const escapeControl = (s) => {
    const control = /[\\\x00-\x1f\x7f-\xff]/g; // eslint-disable-line no-control-regex
    if (control.test(s)) {
...
export const getEnumAttr = (name, value, regex) => {
    if (!regex.test(value))
        throw new Error('Parameter "' + name + '" is invalid');
    return ' ' + name + '="' + value + '"';
};
export const getBoolAttr = (name, value) => {
    return ' ' + name + '="' + value + '"';
};
export const getIntAttr = (name, value, min, max) => {
    if (isNaN(value) || value < min || value > max)
        throw new Error('Parameter "' + name + '" is invalid');
    return ' ' + name + '="' + value + '"';
};
export const getUByteAttr = (name, value) => {
    return getIntAttr(name, value, 0, 255);
};
export const getUShortAttr = (name, value) => {
    return getIntAttr(name, value, 0, 65535);
};
export const getShortAttr = (name, value) => {
    return getIntAttr(name, value, -32768, 32767);
};
export const getEnumIntAttr = (name, value, regex, min, max) => {
