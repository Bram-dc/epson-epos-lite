export const uint8 = (int) => {
    if (int > 255)
        throw new Error(`${int} is not a valid uint8`);
    return int;
};
export const uint3 = (int) => {
    if (int > 7)
        throw new Error(`${int} is not a valid uint8`);
    return int;
};
export const encodeCharacter = (character) => character.charCodeAt(0);
const _ = encodeCharacter;
export const selectSpecialCharacter = (table, code) => [ESC, _('t'), table, code, ESC, _('t'), 0];
export const escapeCharacter = (character) => {
...
export const encodeString = (string) => string.split('').reduce((array, character) => { array.push(...escapeCharacter(character)); return array; }, []);
